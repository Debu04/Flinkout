...
export async function syncActivity(clientId: string, expectedOwnerId?: string): Promise<LocalActivity | undefined> {
  ...
  try {
    const response = await api<SyncResponse>('/activities/sync', { method: 'POST', body: JSON.stringify({ clientId: syncing.clientId, type: syncing.type, visibility: syncing.visibility, startedAt: syncing.startedAt, endedAt: syncing.endedAt, durationS: syncing.elapsedBeforePauseS, movingTimeS: Math.round(syncing.movingTimeS ?? syncing.elapsedBeforePauseS), distanceM: syncing.distanceM, steps: syncing.steps ?? 0, stepSource: syncing.stepSource ?? 'UNAVAILABLE', averagePaceSPerKm: syncing.averagePaceSPerKm ?? null, caloriesKcal: syncing.caloriesKcal ?? 0, currentElevationM: syncing.currentElevationM ?? null, elevationGainM: syncing.elevationGainM ?? 0, elevationLossM: syncing.elevationLossM ?? 0, distanceSource: syncing.distanceSource ?? 'NONE', route: syncing.route }) });
