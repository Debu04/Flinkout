...
export function MobileMapExperience() {
  ...
  const people = mode === 'CONNECTED' ? [] : previewPeople;
  const activities = mode === 'CONNECTED' ? [] : previewActivities;

  async function loadNearby(nextCenter: [number, number], nextRadius: number, announce = true) {
    setLoading(true);
    setError('');
    try {
      ...
      setJoinedIds(response.live.filter(session => session.joinedByViewer).map(session => session.id));
      if (announce) notify(response.live.length ? `Found ${response.live.length} live ${response.live.length === 1 ? 'activity' : 'activities'} nearby.` : 'No active live activities were found in this area.');
    } catch (cause) {
      setLiveSessions([]);
