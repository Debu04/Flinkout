...
function ProfileMovementCard({ activity }: { activity: SocialActivity }) {
  const metadata = getDemoActivity(activity.id);
  ...
  return <article className="profile-movement-card">
    <Link href={`/activities/${activity.id}`} className={`profile-route-art ${activity.type.toLowerCase()}`}><span>{activity.type}</span><svg viewBox="0 0 200 70" aria-label={`${metadata.title} route preview`} role="img"><path d="M8 55 L48 32 L88 43 L121 13 L192 29" fill="none" stroke="currentColor" strokeWidth="4" strokeLinecap="round" /></svg></Link>
    <div className="profile-movement-body"><header><div><h3>{metadata.title}</h3><p>{new Date(activity.startedAt).toLocaleDateString()} - {metadata.location}</p></div><button aria-label={saved ? 'Remove saved activity' : 'Save activity'} aria-pressed={saved} onClick={() => { toggleSaved(activity.id); notify(saved ? 'Removed from saved activities.' : 'Activity saved for later.'); }}><UiIcon name="bookmark" /></button></header>
...
export function UserProfile({ username }: { username: string }) {
  ...
  const totalDistance = activities.reduce((sum, activity) => sum + activity.distanceM, 0) / 1000;
  ...
  return <section className="public-profile-page">
    ...
    <section className="public-profile-hero">
      ...
      {user.isSelf ? <Link className="follow-button" href="/profile/edit">Edit profile</Link> : <button className="follow-button" onClick={() => void follow()}>{effectiveFollowing ? 'Following' : 'Follow'}</button>}
      {user.profile?.bio && <p className="public-profile-bio">{user.profile.bio}</p>}
    </section>
    <section className="public-profile-stats"><span><small>Activities</small><strong>{activities.length}</strong></span><span><small>Total distance</small><strong>{totalDistance.toFixed(1)} <em>km</em></strong></span><span><small>Streak</small><strong>{preview ? demo.streakDays : 0} <em>Days</em></strong></span></section>
    <section className="recent-movement"><header><h2>Recent Movement</h2><Link href="/explore">Explore more</Link></header>{activities.length ? activities.map(activity => <ProfileMovementCard key={activity.id} activity={activity} />) : <div className="card empty-state"><h3>No public activities yet</h3><p>Shared movement will appear here.</p></div>}</section>
